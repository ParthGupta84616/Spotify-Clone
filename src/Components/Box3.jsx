import React from 'react';
import VolumeBar from './VolumeBar';

function Box3() {
  return (
    <div className='flex flex-col items-center justify-center p-8 sm:flex-row'>
      <div className="playlist transform transition duration-300 hover:scale-110 mb-4 sm:mb-0 sm:mr-4">
        <img src="data:image/png;base64," alt=''/>
      </div>
      <div className="Volume size-8 transform transition duration-300 hover:scale-110 mb-4 sm:mb-0 sm:mr-4">
        <img src="data:image/png;base64," alt=''/>
      </div>
      <div className="volume">
        <VolumeBar />
      </div>
    </div>
  );
}

export default Box3;
